...
As of 2026-07-21, this behavior is pinned by a permanent regression test in `checker.py`'s selftest (`mixed_unit_baseline_false_block`): a goal-race plan with a distance-stated baseline and a minute-stated race week must BLOCK. The test asserts the limitation, not its correctness — any future change to this behavior now has to be deliberate, visible in the selftest, and re-verified against every committed ledger.
