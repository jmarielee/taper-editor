...
- python3 checker.py --selftest — 8/8, deterministic, including a clean control case that must not fire and a regression test that pins the Specimen 6 mixed-unit false block as documented, known behavior (receipts: runs/2026-07-19-selftest-run.txt at 6/6 and runs/2026-07-20-selftest-run.txt at 7/7 before the pin was added 2026-07-21, runs/2026-07-21-selftest-run.txt at 8/8 after).
- `python3 check_contract.py` — a second deterministic gate, over the **output contract** rather than the arithmetic. `checker.py` proves the math; this proves the shape every report promises. It blocks any report where a finding drops one of its four parts, where a "Your call" hands back anything but a question (a rewrite, a prescription, a bare statement), or where a "Your plan says" quote can't be found — as a contiguous run of words — in that specimen's committed evidence file. Whitespace, table delimiters, emoji, and dash style are flattened for matching; the words and their order are not. `--selftest` proves the gate itself (8/8: clears clean reports, blocks non-questions, fixes-instead-of-questions, fabricated quotes, reordered quotes, and missing parts). Run against the repo it clears all five reports and verifies 25 quotes against source; the nyc report ships no source, by the anonymous witness's terms, and is reported SKIPPED — not silently passed. The no-rewrite, always-a-question rule is therefore a constraint the repo enforces on itself, not a promise in prose.
- Every checker-backed specimen's committed ledger reproduces its receipt in `runs/` exactly, on demand (timestamp and final-state scoping noted in Quickstart). Specimen 5 is a model-only scope-decline session and therefore has no ledger or machine receipt.
...
checker.py                           the deterministic rule engine (the arithmetic)
check_contract.py                    the deterministic gate over the output contract (four-part shape, question handoff, verbatim-quote check)
